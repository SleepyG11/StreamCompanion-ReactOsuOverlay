...
function mergeObjects(vueThis, target, source) {
	for (const [key, value] of Object.entries(source)) {
		...
		else vueThis.$set(target, key, value);
	}
}
...
export function watchTokens(tokenList, onTokensUpdated, bulkTokenUpdateType, updatesPerSecond) {
	...
	rws.AddTokens = (tokens) => {
		...
		if (rws.readyState === 1) rws.send(JSON.stringify(rws.watchedTokens));
	};

	rws.AddToken = (token) => {
		if (rws.watchedTokens.includes(token)) return;
...
function _GetToken(rws, tokens, tokenName, decimalPlaces) {
	if (tokens.hasOwnProperty(tokenName)) {
		if (decimalPlaces !== undefined && decimalPlaces !== null) return Number(tokens[tokenName]).toFixed(decimalPlaces);
		return tokens[tokenName];
	}
	if (rws.watchedTokens.indexOf(tokenName) === -1) rws.AddTokens([tokenName]);
	return '';
}
...
function _IsInStatus(rws, tokens, osuStatuses) {
	if (Array.isArray(osuStatuses)) return osuStatuses.indexOf(_GetToken(rws, tokens, 'status')) > -1;
...
function ColorFromDotNetColor(netColor) {
	return `#${netColor.R.toString(16).padStart(2, '0')}${netColor.G.toString(16).padStart(2, '0')}${netColor.B.toString(16).padStart(
		2,
		'0'
	)}${netColor.A.toString(16).padStart(2, '0')}`;
}

function ApplyModsToTime(time, modsEnum) {
	if (typeof time === String) time = Number(time);

	if (typeof modsEnum === String) modsEnum = Number(modsEnum);

	// DT & NC
	if ((64 & modsEnum) != 0 || (512 & modsEnum) != 0) time /= 1.5;

	// HT
	if ((256 & modsEnum) != 0) time /= 0.75;
